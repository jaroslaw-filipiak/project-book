<template>
  <div class="story-view container mx-auto px-4 py-8">
    <h1 class="text-3xl font-bold mb-6">Stwórz indywidualną historię</h1>
    
    <CreatorNavigation currentStep="historia" />

    <div class="story-container">
      <p class="text-gray-600 mb-6">
        Uzupełnij pytania do każdej postaci aby zbudować swoją własną historię
      </p>
      ...
      <div v-if="!areAllStoriesComplete" class="navigation mt-8 flex justify-between">
        <button @click="router.push('/kreator/postacie')" class="px-4 py-2 border rounded">
          Back to Characters
        </button>
      </div>
    </div>
...
onMounted(() => {
  // If no characters are created, redirect back to characters page
  if (characterStore.characters.length === 0) {
    router.push('/kreator/postacie')
    return
  }
  
  // Initialize the first character if none is selected
  if (!bookStore.currentStoryCharacterId && characterStore.characters.length > 0) {
    const firstCharacter = characterStore.characters[0]
    bookStore.setCurrentStoryCharacter(firstCharacter.id, firstCharacter.sex)
  }
})
...
const goToNextQuestion = () => {
  ...
  if (isLastQuestion.value) {
    ...
    if (isLastCharacter.value) {
      navigateNext() // Move to next main step (pages)
    } else {
      // Move to the next character
      const nextCharacterIndex = currentCharacterIndex.value + 1
      if (nextCharacterIndex < characterStore.characters.length) {
        const nextCharacter = characterStore.characters[nextCharacterIndex]
        // Load the next character's questions
        bookStore.setCurrentStoryCharacter(nextCharacter.id, nextCharacter.sex)
      }
    }
  } else {
...
// Navigate to next step (pages)
const navigateNext = () => {
  if (areAllStoriesComplete.value) {
