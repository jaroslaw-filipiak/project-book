<template>
  <div class="character-preview">
    ...
    <div v-if="character" class="character-info">
      ...
      <div v-if="character.isComplete" class="complete-badge">
        ✓ Complete
      </div>
    </div>
  </div>
...
const renderBody = computed(() => {
  ...
  return bookStore.getCharacterBodySvg(props.character.body)
})

const renderFace = computed(() => {
  if (!props.character?.face) return ''
  return bookStore.getCharacterFaceSvg(props.character.face)
})

const renderHair = computed(() => {
  if (!props.character?.hair) return ''
  return bookStore.getCharacterHairSvg(props.character.hair)
})
</script>
